#!/bin/bash
#SBATCH --job-name=RT_LCC04HCP # specifies a user-defined job name
#SBATCH --nodes=5 # number of compute nodes to be used
#SBATCH --ntasks-per-node=8 # number of MPI processes per node
#SBATCH --partition=small_fat # partition (small_shared, small, medium, small_fat, small_gpu)
# special partitions: large (for selected users only!)
# job configuration testing partition: dev
#SBATCH --cpus-per-task=9 # number of cores per process
...
mpirun -np 40 $HOME/AutoPas/build-MPI2/examples/md-flexible/md-flexible --yaml-filename rayleighTaylor.yaml
