#!/bin/bash
#SBATCH --job-name=Empty_Job_Array # specifies a user-defined job name
...
#SBATCH --time=03:00:00 # maximum wall clock limit for job execution
...
cd ${thread_count[${SLURM_ARRAY_TASK_ID}]}_threads/SpT_${verlet_skin_per_timestep[${SLURM_ARRAY_TASK_ID}]}/ #run_${run[${SLURM_ARRAY_TASK_ID}]}
...
do
    ...
    mpirun -np 1 $HOME/AutoPas/build-MPI/examples/md-flexible/md-flexible --use-tuning-log true --yaml-filename input.yaml > logOutput_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out
    cd ..
done
