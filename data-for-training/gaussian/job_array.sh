#!/bin/bash
#SBATCH --job-name=Gauss_Job_Array # specifies a user-defined job name
...
cd noClusters_${noClusters[${SLURM_ARRAY_TASK_ID}]}/${thread_count[${SLURM_ARRAY_TASK_ID}]}_threads/SpT_${verlet_skin_per_timestep[${SLURM_ARRAY_TASK_ID}]}/run_${run[${SLURM_ARRAY_TASK_ID}]}
