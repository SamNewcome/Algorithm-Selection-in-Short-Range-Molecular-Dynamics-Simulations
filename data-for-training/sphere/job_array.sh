#!/bin/bash
#SBATCH --job-name=Sphere_Job_Array # specifies a user-defined job name
...
declare -a run
index=0
for radius_iter in 05 10 15 20
do
...
cd radius_${radius[${SLURM_ARRAY_TASK_ID}]}/spacing_${spacing[${SLURM_ARRAY_TASK_ID}]}/${thread_count[${SLURM_ARRAY_TASK_ID}]}_threads/SpT_${verlet_skin_per_timestep[${SLURM_ARRAY_TASK_ID}]}/ #run_${run[${SLURM_ARRAY_TASK_ID}]}
